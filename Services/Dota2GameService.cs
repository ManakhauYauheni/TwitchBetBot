...
namespace TwitchBetBot.Services
{
    public class Dota2GameService : INotifyPropertyChanged
    {
        ...
        private bool _roshanKilled = false;
        public bool WasRoshanKilled => _roshanKilled;
        ...
        private bool IsReplayMatch(GameState gs)
        {
            ...
            {
                LogToUI($"Ошибка проверки реплея: {ex.Message}");
                return false;
            }
        }
        ...
        private string GetPlayerTeamFromGSI(GameState gs)
        {
            ...
            {
                LogToUI($"Ошибка определения команды: {ex.Message}");
            }
            return "";
        }
        private void OnNewGameState(GameState gs)
        {
            try
            {
                ...
                if (!_roshanKilled && _isInGame && gs.Events != null)
                {
                ...
                else if (gameState.Contains("GAME_IN_PROGRESS"))
                {
                    if (!_isInGame)
                    {
                        ...
                        {
                            LogToUI("Игнорируем GAME_IN_PROGRESS после отмены", false);
                            return;
                        }
                        _undefinedStartTime = null;
                        _disconnectDetected = false;
        ...
        private void OnTeamVictory(TeamVictory gameEvent)
        {
            try
            {
                ...
                {
                    LogToUI("TeamVictory: нет данных о команде");
                    return;
                }
        ...
        private void DetermineWinnerByOtherMethods()
        {
            try
            {
                ...
                {
                    LogToUI("Нет данных для определения победителя");
                    return;
                }
                var map = _gsl.CurrentGameState.Map;
                LogToUI("Использую счёт для определения победителя");
        ...
        private async Task EndGame()
        {
            ...
            try
            {
                ...
                LogToUI($"Длительность: {matchToEnd.Duration:mm\\:ss}");
        ...
        private async Task UpdateStatsAfterGame(Dota2Match match)
        {
            try
            {
                ...
                // Персистим MMR и статистику в конфиг, чтобы не терялись при перезапуске
                _viewModel.SaveMmrToConfig();
            }
            catch (Exception ex)
            {
